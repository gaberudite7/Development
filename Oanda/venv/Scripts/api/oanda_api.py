...
class OandaApi:
    ...
    def get_account_ep(self, ep, data_key):
        ...
        ok, data = self.make_requests(url);
    ...
    def fetch_candles(self, pair_name, count=10, granularity="H1", price="MBA", date_f=None, date_t=None): # pair EURUSD, count number of candles, granularity is hours 1
        ...
        ok, data = self.make_requests(url, params=params)
    ...
    def get_candles_df(self, pair_name, **kwargs):
        ...
        for candle in data:
            new_dict = {}
            # safe parsing of time and volume (fall back to None if missing)
            time_val = candle.get('time')
            new_dict['time'] = parser.parse(time_val) if time_val is not None else None
            new_dict['volume'] = candle.get('volume')

            for p in prices:
                price_dict = candle.get(p)
                if not price_dict:
                    continue
                for o in ohlc:
                    v = price_dict.get(o)
                    new_dict[f"{p}_{o}"] = float(v) if v is not None else None
    ...
    def get_candles_df_broken(self, pair_name, **kwargs):
    ...
    def get_candles_df_download(self, pair_name, **kwargs):
    ...
    def place_trade(self, pair_name: str, units: float, direction: int, stop_loss: float=None, take_profit: float=None):
        ...
        if stop_loss is not None:
            ...
            data['order']['stopLossOnFille'] = sld
        ...
        ok, response=self.make_requests(url, verb="post", data=data, code=201)

        #print(ok, response)

        if ok == True and 'orderFillTransation' in response:
    ...
    def close_trade(self, trade_id):
        ...
        ok, response = self.make_requests(url, verb="put", code=200) #close is a put trade in Oanda API
    ...
    def get_open_trade(self, trade_id):
        ...
        ok, response = self.make_requests(url)
