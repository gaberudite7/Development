...
def save_file(final_df: pd.DataFrame, file_prefix, granularity, pair):
    filename = fr"{file_prefix}\{pair}_{granularity}.pkl"
    final_df.drop_duplicates(subset=['time'], inplace=True)
    final_df.sort_values(by=['time'], inplace=True)
    final_df.reset_index(drop=True, inplace=True)
    final_df.to_pickle(filename)
    s1 = f"***{pair} {granularity} {final_df} --> {final_df.time.max()}"
...
def fetch_candles(pair, granularity, date_f: dt.datetime, date_t: dt.datetime, api: OandaApi):
    ...
    while attempts < 3:
        ...
        if candles_df is not None:
            ...
            print(f"While loop to fetch api failed after 3 attempts")
        attempts += 1
...
def collect_data(pair, granularity, date_f: dt.datetime, date_t: dt.datetime, file_prefix, api: OandaApi):
        ...
        for p2 in our_curr:
            ...
            if pair in ic.instruments_dict.keys():
                for granularity in ["M5", "H1", "H4"]:
                    ...
                    collect_data(pair, granularity, "2016-01-07T00:00:00Z", "2021-12-31T00:00:00Z", r'C:\Development\Oanda\Data', api)
