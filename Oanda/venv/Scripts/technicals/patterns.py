...
def set_candle_patterns(df_an: pd.DataFrame):
    # 2 red to 1 green means trend reversal (buy) - hammer
    # 2 green to 1 red is hanging man
    df_an['HANGING_MAN'] = df_an.apply(apply_hanging_man, axis=1)
    # 2 red to green (inverted hammer (buy))
    # 2 green to red (shooting star (sell)
    df_an['SHOOTING_STAR'] = df_an.apply(apply_shooting_star, axis=1)
    # red/green middle (neutral)
    df_an['SPINNING_TOP'] = df_an.apply(apply_spinning_top, axis=1)
    # 100% of candle range (red) - sell
    # 100% of candle range(green = buy)
    df_an['MARUBOZU'] = df_an.apply(apply_marubozu, axis=1)
    # e.g downward trend where green candle ENGULFS prior red candle (is bigger than prior red candle)
    df_an['ENGULFING'] = df_an.apply(apply_engulfing, axis=1)
    # trend upward but at top the candle is green at bottom and next one red at bottom (have similar characteristics - suggests sell)
    df_an['TWEEZER_TOP'] = df_an.apply(apply_tweezer_top, axis=1)
    df_an['TWEEZER_BOTTOM'] = df_an.apply(apply_tweezer_bottom, axis=1)
    # downward trend and small bodied candle (suggests indecision) followed by a reversal green candle and above midpoint of the 2nd to last candle
