...
class MeanRevBacktester():
    ...
    def get_data(self):
        ...
        '''
        raw = pd.read_csv(r"C:\Development\Trader_Workstation\Udemy\Part3\intraday_pairs.csv", parse_dates = ["time"], index_col = "time")
...
if __name__ == "__main__":
    # parameters are sma and # of standard deviations as well as trading costs (in percentage percentage trading cost = ptc)
    ptc = 0.00007
    tester = MeanRevBacktester("EURUSD", 30, 2, "2018-01-01", "2019-12-31", ptc)

    # Compares performance to buy and hold
    print(tester.test_strategy())
    print(tester.plot_results())

    # SMA (short term moving average range of 25-100 days with 1 step)
    # standard deviation range 1-5 with 1 step
    # should output the days, deviations that should be selected as well as the returns
